...
class Garatge(object):
    ...
    def __iter__(self):
        return iter(self.getGaratge())

    def keys(self):
        return self.getGaratge().keys()

    def items(self):
        return self.getGaratge().items()

    def values(self):
        return self.getGaratge().values()

    def __setitem__(self, key, value):
        if(isinstance(value, Cotxe)):
            self.getGaratge()[key] = value
            return
        print "El valor ha de ser de la classe cotxe"

    def __getitem__(self, item):
        if (item in self.getGaratge()):
            return self.getGaratge()[item]


        print "El garatge no te aquesta placa: ", item
        return ""
    ...
    def __delitem__(self, key):
        if(key in self.getGaratge()):
            self.getGaratge()[key] = Garatge.CotxeBuit
            return

        print "El garatge no te aquesta placa: ", key

    def __len__(self):
        return len(self.getGaratge())
    ...
    def ordenaAlfabeticament(self):
        counter = 0
        for cot in sorted(self.getGaratge().values(), key=lambda x: x.getMat().lower()):
            if(cot != Garatge.CotxeBuit):
                self.getGaratge()[counter] = cot
                counter+=1

        while counter < len(self):
            self.getGaratge()[counter] = Garatge.CotxeBuit
            counter+=1
